...
const ncSchema = require('../schemas');

MongoClient.connect(mongoConfig.url, (err, mongoPool) => {
  assert.equal(err, null);

  const mongodb = require('./../database/mongodb')(mongoPool);

  // Definindo a rota no express
  app.use('/graphql', (req, res) => {
    // Funções de batching da biblioteca DataLoader
    // Loaders vão ser inicializados a cada request
    const loaders = {
